/*
definitie sketchRelationPropsDlg klasse.
De standaard eigenschappen dialoog van Relations.

Editor voor het toevoegen van Relations
Based on configurationDefEditor and configurationPropsDlg
*/


default_defName('New sketch relation definition').


:-pce_begin_class(sketchRelationPropsDlg,
		  propertyDialog,
		  "Standard relation properties dialog"
		 ).

variable(arg1,sketchConceptElement,both,"The associated first argument").
variable(arg1Route, chain, both, "Route to the first concept").
variable(arg2,sketchConceptElement,both,"The associated second argument").
variable(arg2Route, chain, both, "Route to the second concept").
variable(readOnly,bool,both).
variable(def,sketchRelationDefinition*,get, 
		"the edited sketchRelationDefinition"). %@nil betekent: nieuwe
...
initialise(D,F: frame):->
	...
	D->+initialise('Relation properties - Sketch', F,later),
	D->>readOnly(@off),
	...
	DefList->>label('Relation definitions:'),
	...
	Add *= imgButton(add, tt:='Add sketch relation definition'),

	Remove *= imgButton(remove, tt:='Delete selected sketch relation definition'),
	...
	get(@model, getModelNameForEditor, 'Relation properties - Sketch', ModelNameForEditor),
...
fillCurrentDefData(D):->
	"fill the editor with saved data for current relation definition selection" ::
...
%%
newObject(D,
	  Arg1: sketchConceptElement, %Concept argument 1
		Arg1Route: chain,
	  Arg2: sketchConceptElement, %Concept argument 2
		Arg2Route: chain
	 ):->
	"Open the dialog for a new relation" ::

	%Als er geen sketchRelationDefinitions zijn, openen we toch, want vanaf hier kan je naar de def-editor
	D->>arg1(Arg1),
	...
	D->>element(@nil),
	get(@model, getModelNameForEditor, 'Add a new relation - Sketch', ModelNameForEditor),
	D->>label(ModelNameForEditor),
	D->>helpId('Sketch_AddNewRelation'), %gp3 0.3.13
...
editObject(D,
	...
	"Open dialog for existing relation" ::
	...
	get(@model, getModelNameForEditor, string('Relation properties%s - Sketch',when(ReadOnly == @on,' [Read Only]',''))?value, ModelNameForEditor),
	D->>label(ModelNameForEditor),
	D->>helpId('Sketch_RelationProperties'), %gp3 0.3.13
...
%
fillDefList(D):->
	"(re)fill the relation definitions list" ::

	List = D<<-defList_member,
	List->>clear,
	% Defs = @model<<-sortedRelationDefinitions,
	Defs = D?sketch<<-sortedSketchRelationDefinitions,
...
saveNewElement(D):->
        ...
        @nil = D<<-selectedDef,
        send(D, notChangedRelationDef), !, fail. 


%%
saveNewElement(D):->
        % first, create and save a relation definition, if necessary
        D->>saveDef,
        D->>fillCurrentDefData, %opnieuw inlezen
	% then, make a new relation
	@model->>changeRequest(newRelation,
		D?sketch,
		D?editor,
...
saveChangedElement(D):->
        ...
        D->>saveDef,
        D->>fillCurrentDefData, %opnieuw inlezen
	@model->>changeRequest(changeRelation,
		D?sketch,
		D?editor,
		D?element, % old Relation?
		% D?state_member?selection,
		% D?element?definition,  % new Relation definition? OR
		D?selectedDef, % new Relation?
		% D?def, % new Relation?
...
%%
notChangedRelation(D):->
	"Succeeds when the relation is not changed by the user" ::
	%we beschouwen het als gewijzigd als het sowieso anders, is, geen gedoe met makeGarp op dit nivo
...
notChanged(D):->
	"Succeeds when the relation is not changed by the user" ::
        ...
        % When the RelationDef, and the Relation itself have not been changed
	D->>notChangedRelationDef, 
	D->>notChangedRelation.
...
checkSaveSelection(D):->
	...
	D->>notChangedRelationDef,!. %niets aan de hand
%
checkSaveSelection(D):->
	...
	Dlg->>application(@app),
	Dlg->>append(text('You made changes to this relation definition. Do you want to save them?')),
...
saveDef(D):->
	...
	D->>notChangedRelationDef,!. %niets te doen
%
saveDef(D):->
	...
	if
		@nil = D<<-def
	then
                @model->>changeRequest(addRelationDef,
				@model,  % not sure if this should be changed to D?sketch, AB, nov 2006
				D,
				D?name_member?selection,
				D?remarks_member?contents)
	else
		@model->>changeRequest(changeRelationDef,
				D?def,
				D,
				D?name_member?selection,
				D?remarks_member?contents).
...
selectedDef(D,
	...
	"Return selected relation definition or @nil" ::
...
%
changeApplied_addRelationDef(D,
	CR : changeRequestor):->
     debug(sketch(relations), 'changeApplied_addRelationDef', []), 
	%hervullen vd lijst en de goede weer selecteren
	% Def = D<<-selectedDef,
	% Def = D<<-def,
	D->>fillDefList,
     get(CR, result, R), 
     debug(sketch(relations), 'changeApplied_addRelationDef set selection to CR?result: ~w', [R]), 
...
%
changeApplied_changeRelationDef(D,
	_CR : changeRequestor):->
	
     debug(sketch(relations), 'changeApplied_changeRelationDef', []), 
...
%%
changeApplied_deleteRelationDef(D,
	CR : changeRequestor
	):->
...
%%
changeApplied_changeRelation(D,
	CR: changeRequestor):->
	...
	% D?editor->>deleteUnusedDefs,
	D->>return.
%%

%%
changeApplied_deleteRelation(D,
	CR: changeRequestor):->
