/*
Definitie sketchGenericRelationVisualElement class:

abstracte parent van classes voor grafische weergave van relaties in
sketches. Deze zorgt ervoor dat het relatie element zelf (dus een
...
:-pce_begin_class(sketchGenericRelationVisualElement,
		  sketchVisualElement,
...
setArguments(VE,
	...
	if
		_OldArg1 = VE<<-firstArgument
	then (
		VE->>delete_hypers(arg1)
		),

	if
		_OldArg2 = VE<<-secondArgument
	then (
...
doDisplay(VE,
	...
	then
		...
		Arg1Point = Arg1<<-absolute_position
	        , 
	        debug(sketch(layout), 'Arg1Point = Arg1<<-absolute_position\n',[])
	)
	else
        (
		Arg1Point *= point(VisArea?left_side,VisArea?top_side),
	        debug(sketch(layout), 'Arg1Point = point(VisArea?left_side,VisArea?top_side)\n',[])
	),
	...
	then
		...
		Arg2Point = Arg2<<-absolute_position
	        , 
	        debug(sketch(layout), 'Arg2Point = Arg2<<-absolute_position\n',[])
	)
	else
        (
		Arg2Point *= point(VisArea?right_side,VisArea?bottom_side),
	        debug(sketch(layout), 'Arg2Point = point(VisArea?left_side,VisArea?top_side)\n',[])
	),
...
savedPosition(VE,
	...
	%gp3 1.0: overwrite of sketchVisualElement
	...
	Position = ?(VE?firstArgument?absolute_position,mid,VE?secondArgument?absolute_position)<<-plus(?(VE?sketch,layOutInfo,VE?sketchElement, relPosition,VE?route)).
%%
...
%%
canHide(_VE):->
	% relations can not be hidden in Sketch
	false.
%%
